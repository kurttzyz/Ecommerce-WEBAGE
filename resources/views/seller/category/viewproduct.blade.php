    ...
    <div class="relative bg-black border border-gray-200 shadow-2xs rounded-xl dark:bg-neutral-900 dark:border-neutral-700 dark:shadow-neutral-700/70 animate-fade-in">
        ...
        <div class="absolute top-0 start-0 end-0">
          <div class="p-4 md:p-5">
            <center><p class="mt-1 text-white-800"><strong>Mentor's Name:</strong> {{ $product->store->store_name }}</p> </center>
            @if (session('success'))
            <script>
                Swal.fire({
                    icon: 'success',
        ...
        <div class="mt-6">
            ...
            <form action="{{ route('seller.product.review.store', $product->id) }}" method="POST" class="mt-2 space-y-2">
                @csrf
