...
<head>
	...
	<title>@yield('title_customer')</title>
...
<body class="bg-black text-white font-sans antialiased">
		...
		<nav id="sidebar" class="sidebar js-sidebar">
			<div class="sidebar-content js-simplebar">
				...
				<ul class="sidebar-nav">
					...
					<li class="sidebar-item {{ request()->routeIs('customer.class') ? 'active' : '' }}">
						<a class="sidebar-link relative" href="{{ route('customer.class') }}">
							<i class="align-middle" data-feather="sliders"></i>
	...
	$total_classes = Auth::user()->enrolled_courses->count(); // Note the underscore
					...
					<a class="sidebar-link" href="{{ route('certificates') }}">
						<i class="align-middle" data-feather="sliders"></i>
	...
	$total_classes = Auth::user()->certificates()->count(); // Note the underscore
				...
				<li class="sidebar-item {{ request()->routeIs('achievements') ? 'active' : '' }}">
					<a class="sidebar-link" href="{{ route('achievements') }}">
						<i class="align-middle" data-feather="sliders"></i>
				...
				<div class="container">
					...
					<div class="container mx-auto px-6 flex justify-between items-center">
						...
						<div class="flex items-center space-x-8 text-lg">
							<a id="homepage-link" href="{{route('customer.dashboard')}}" class="text-black flex items-center hover:text-red-500 no-underline">
								<i class="align-middle hover:text-red-500" data-feather="home"></i> Home
							</a>
							
							<a id="shop-link" href="{{ route('customer.store.view') }}" 
...
<script>
	$('#shop-link').on('click', function(event) {
		...
		const url = '{{ route('customer.store.view') }}';
		$.ajax({
			url: url,
			method: 'GET',
	...
	$('#homepage-link').on('click', function(event) {
		...
		event.preventDefault();
		const url = '{{ route('customer.dashboard') }}';
		$.ajax({
			url: url,
			method: 'GET',
			success: function(response) {
